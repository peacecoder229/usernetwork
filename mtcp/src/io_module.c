...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
	...
	if (current_iomodule_func == &ps_module_func) {
...
#ifndef DISABLE_DPDK
		int cpu = (CONFIG.num_cores == 0) ? GetNumCPUs() : CONFIG.num_cores;
		mpz_t _cpumask;
		char cpumaskbuf[32] = "";
		char mem_channels[8] = "";
		...
		socket_mem = 
			RTE_ALIGN_CEIL((unsigned long)ceil((cpu *
							    (CONFIG.rcvbuf_size +
							     CONFIG.sndbuf_size +
		...
		/* STEP 5: finally, verify whether all the argvs are valid */
		for (i = 0; i < argc; i++)
			TRACE_INFO("argv[%d]: %s\n", i, argv[i]);
		...
		num_queues = MIN(num_cpus, MAX_CPUS);
	...
	for (i = 0; i < CONFIG.eths_num; ++i) {
		...
		if (strstr(port_stat_list, CONFIG.eths[i].dev_name) != NULL)
