		...
		const struct sockaddr *addr, socklen_t addrlen)
{
	...
	/* take care mTCP<->Ethernet flow directory logic */
	if (CONFIG.eth_filter && mtcp->iom->dev_ioctl != NULL) {
		int nif;
		int rc;
		uint8_t is_external;

		/* First determine which NIC should the filter be added to */
		nif = GetOutputInterface(addr_in->sin_addr.s_addr, &is_external);
		if (nif < 0) {
			TRACE_ERROR("Failed to identify which NIC needs to be attached"
				    " (port %hu, queue: %d)\n",
				    ntohs(addr_in->sin_port), mtcp->ctx->cpu);
			errno = ENOSYS;
			return -1;
		}
		/**
		 * Forcefully sleep so that other processes/threads
		 * don't create a race condition 
		 */
		sleep(mtcp->ctx->cpu);
		/* attach the filter */
		rc = mtcp->iom->dev_ioctl(mtcp->ctx, nif,
					  ETH_ATTACH_FILTER,
					  (struct sockaddr *)addr);
		if (rc == -1) {
			TRACE_ERROR("Failed to attach Ethernet flow director filter"
				    " (port %hu, queue: %d)!\n",
				    ntohs(addr_in->sin_port), mtcp->ctx->cpu);
			errno = ENOSYS;
			return -1;
		} else {
			TRACE_API("Everything was successful!!. "
				  "Bound to port %hu for queue: %d\n",
				  ntohs(addr_in->sin_port), mtcp->ctx->cpu);
		}
		UNUSED(is_external);
	}
